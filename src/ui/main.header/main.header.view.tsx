    ...
    <header>
      <div className="row">
        <div className="col-md-12">
          <HeadingComponent heading="h1">{MAIN_HEADER.TITLE}</HeadingComponent>
          ...
          <form id="cspForm" onSubmit={$this.checkStatus}>
            <textarea
              name="cspImportTextArea"
              rows={5}
              id="cspBreakdownTextArea"
              className="csp-breakdown-text-area form-control"
              placeholder={MAIN_HEADER.IMPORT_PLACEHOLDER}
              aria-label={MAIN_HEADER.IMPORT_PLACEHOLDER}
            />

            <div className="row import-row">
              <button
                id="cspBtn"
                className={`btn btn-primary ${$this.getImportStatus(
                  $this.state.importCompleted
                )}`}
                disabled={
                  $this.state.importCompleted &&
                  !$this.state.reImportCheckbox.checked
                }
              >
                {MAIN_HEADER.IMPORT_BTN}
              </button>
              {$this.state.importCompleted && (
